...
Here is the default *sysconfig.toml* for pixhawk FMUv5. Be noticed the target value should match with BSP's target name (defined in *bsp.h*). Otherwise the system won't boot because of the assertion failue to prevent using of a wrong configuration.
